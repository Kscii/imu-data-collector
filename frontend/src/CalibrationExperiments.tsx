...
import { OrientationSetup } from "./OrientationSetup";
...
function warningText(code: string): string {
  ...
  const messages: Record<string, [string, string]> = {
    raw_axis_setup_conflicts_with_fitting_faces: ["六面拟合与先前原始轴识别不一致，请检查方向并重新建实验。", "Fitting faces disagree with the original axis setup. Check the directions and create a new experiment."],
...
export function CalibrationExperimentPage({ sensorSn, profiles, onSelectSensor, onBack, onOpenSettings, allowedUnikeys, interactionBlocked, legacy }: { sensorSn: string; profiles: { sensor_sn: string; display_name: string; selectable: boolean }[]; onSelectSensor: (sn: string) => void; onBack: () => void; onOpenSettings: (sn: string) => void; allowedUnikeys: string[]; interactionBlocked: boolean; legacy?: React.ReactNode }) {
  ...
  return <main className="calibration-page">
    <div><button onClick={onBack}>{tr("返回设备设置", "Back to device settings")}</button></div>
    ...
    {experiment && !active && <section className="panel"><h2>{tr("保存与共享", "Save and share")}</h2><div className="save-row"><a className="button-link" href={`${endpoint}/candidate.yaml`}>{tr("下载候选 YAML", "Download candidate YAML")}</a><button disabled={locked} onClick={() => invoke(async () => { if (!window.confirm(tr("将此设备的草稿参数替换为本报告结果？其他设备保持不变。", "Replace this device's draft parameters with this report? Other devices stay unchanged."))) return; await request(`${endpoint}/copy-to-workspace`, "POST"); setMessage(tr("已复制到本地配置工作区，状态仍为未验证。", "Copied to the local configuration workspace; it remains unverified.")); })}>{tr("保存为配置草稿", "Save as configuration draft")}</button><button disabled={locked} onClick={() => onOpenSettings(experiment.sensor_sn)}>{tr("查看配置草稿", "View configuration draft")}</button><button disabled={locked || experiment.upload?.state === "uploading"} onClick={() => capture("upload")}>{tr("上传实验 / 重试", "Upload experiment / retry")}</button><button disabled={locked} onClick={() => invoke(async () => { const value = await request("/api/v1/cloud/oauth/start", "POST"); window.open(value.authorization_url, "_blank", "noopener"); })}>{tr("Google 登录", "Sign in with Google")}</button></div><p>{tr("上传使用已有团队账号；离线实验无需登录。", "Upload uses your existing team account. Offline experiments do not require sign-in.")} {cloud?.logged_in ? tr("当前已登录。", "Currently signed in.") : ""}</p>{experiment.upload && <p role="status">{experiment.upload.state} · {number((experiment.upload.completed_bytes ?? 0) / 1048576)} / {number((experiment.upload.total_bytes ?? 0) / 1048576)} MiB {experiment.upload.error && userVisibleMessage(experiment.upload.error)}</p>}
