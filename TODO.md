...
- 本仓库负责 CW12EU-T 的真实 IMU/视频采集、同步、标注、校准门禁、版本化导出和不可变团队
  快照；另有独立的合成运动人工审核模块。训练、模型比较与 Android 推理属于其他仓库。
...
- 设备阻塞：当前唯一 CW12EU-T 样机已出现异常发热、无法产生有效 IMU 数据且按键指示灯失效；
  在客户提供替换设备并先完成短时安全探测前，不继续任何需要真实 IMU 的录制或断连实验。
...
- 下列真实采集验收同样等待可安全使用的替换 IMU；CI 构建与模拟测试不能解除该设备阻塞。
...
- [~] 本地 pilot 已从 GCS 测试前缀显示 12 条未审核的真实候选，独立 API/页面、revision 和冻结快照请求已实现；用户界面检查及正式人工决定尚未完成。实施边界和完整台账见
  [合成运动标注模块](docs/synthetic-motion-module.md)。
- [ ] 由真实人工 pass 完成云端小快照、失败恢复和高效批量审核验收，再决定正式部署；不得把本地基础设施测试决定作为正式接受。
