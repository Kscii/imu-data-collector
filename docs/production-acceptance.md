...
- 完成后任务只读且能下载新的 aligned；重开会移除当前导出入口；
- 再次完成生成不同 revision 的不可变 aligned，旧对象不会被误下载；
- 生成训练快照，同时得到逐录制 TAR 和合并的 benchmark `cw12eu.h5`；重复点击相同内容复用同一 snapshot ID；
- “数据集”页显示公共当前版本 `imu_25hz_snapshot_v1` 及 9 个 H5；历史版本默认折叠；团队
  尚无 current pointer 时显示“尚未发布”，而不是 500；
- 下载 `cgu_bes.h5` 后确认 SHA-256 为
  `64357f77ef939dbcf10bdb488f1116bce5b6988c39d31a2e86174449cc8409a2`；
- test 录制能下载原始 H5/review，但不能完成或进入训练快照；
- 校准证据页的 15 条记录可播放、可下载 H5，且不出现在标注队列。
- 校准证据页的原始计数、即时推导 SI 和视频共享同一录制时间游标；点击任一曲线能定位视频，
  播放或拖动视频能定位两条曲线，并能核对当前样本 HEX、十进制原始值和 SI 值。
- 校准证据列表只在自身区域滚动，不得在页面滚动时覆盖右侧视频或实验说明；窄屏时改为上下布局。
- 已完成录制重新进入时默认打开“标注”页而不是“检查”页；完成操作后也停留在“标注”页。
- 使用普通白名单成员账号确认能进入标签管理，并用一组临时同类标签验收创建、改名、停用确认、
  恢复、迁移预览、精确确认、源标签自动停用、完成录制新活动导出、旧快照不变、过期预览冲突
  和未使用标签删除；当前版本必须显示该成员的 UniKey 和变更类型。普通成员仍不能刷新 Bucket
  或清理历史训练快照。
- 使用 `?lang=zh-CN` 与 `?lang=en` 各验收一次；无参数时 `zh-*` 浏览器显示中文，其余语言显示
  英文。两种模式都不得出现另一种语言的按钮、提示、校准语义或 API 错误。
- 下载训练 TAR 后运行 SOFT3888 的 `validate-team`；确认内层 manifest `2.0.0`、aligned HDF5
  `3.1.0`、25 Hz、逐文件大小与 SHA-256 全部通过，再交给 benchmark 拉取。
