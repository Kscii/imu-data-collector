...
脚本会拒绝打断正在录制或正在执行的后台收尾/上传任务，完成锁定依赖安装、前端暂存构建、前后端版本校验、
...
- 无论最大残差是多少，拟合 RMS `> 100 ms`、通知间隔 `> 2 s`、回调丢包、解析失败仍会
  阻止发布。
...
第一个锚点会立即作为可恢复草稿保存，状态保持“尚未评估”；只有开始与结束两个锚点都存在
时才执行正式同步评估。删除任一锚点后也回到草稿，不应因缺少第二个锚点进入“保存失败”。
...
点击“结束录制”后，请求只停止 FFmpeg、冻结并关闭 H5、持久化收尾任务，然后立即恢复设备
预览。视频逐帧探测、无损重封装、H5 写入与完整性校验由单一低优先级 worker 在后台串行完成。
它默认可与下一条采集并行；如果真实设备验收出现输入 FPS 下降或回调丢弃，把
`background_jobs.allow_during_recording` 改为 `false` 后重启。

通过收尾门禁的新 `prod` 录制会自动加入发布队列；`test` 只在操作者明确点击发布时入队。
发布任务先生成无重编码 MP4 浏览代理，再上传 H5、原始 MKV、代理 MP4，最后写 manifest。
完成后不会自动删除本地原件。采集端不再提供同步、标注或训练导出。

标注端独立完成同步和标注；完成操作通过门禁后自动生成不可变 `aligned.h5`。任何 `test`
数据或真实校准缺失都会被硬门禁拒绝。当前 GCS 客户端使用 SDK 的可续传上传、对象写入前置
条件、大小与 SHA-256 校验；
网络中断不能删除本地原件。收尾和发布的当前任务状态保存在 SQLite：首次失败后按
5、30、120 秒自动重试三次（合计最多四次执行），服务重启会把中断的 `running` 任务重新
排队。页面显示当前阶段、尝试次数和最后错误；自动重试耗尽后由操作者点击“重新收尾”或
“重新上传”重置额度。

## 中断与恢复

- 正常录制先写 `.partial.h5` 与 `.partial.mkv`，完整收尾和验证后才改为最终文件名。
- 收尾始终在 `.finalizing.h5/.finalizing.mkv` 临时副本中执行；最终文件改名成功后，录制索引
  与任务完成状态在同一 SQLite 事务提交，最后才清理 `.partial.*`。服务在任一步退出都不会
  把半成品误标为 ready，也不会为了重试先删除原始 partial。
- “重新收尾”只适用于尚未发布且仍保有成对 `.partial.h5/.partial.mkv` 的录制；已发布制品
  不允许由本机静默改写。
