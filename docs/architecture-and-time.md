...
3. **训练冻结层**：在 IMU/视频公共有效区间上插值到严格 25 Hz，保留 activity/onset/impact/exclude 标注，运行质量门禁，写入可复现的三表 HDF5。训练窗口跳过 exclude；原始 H5/MKV 永不被覆盖。
...
- 包时间拟合采用分级门禁：最大残差不超过 200 ms 为正常；大于 200 ms 且不超过 500 ms
  ...
  单独代表整段时轴质量；拟合 RMS 大于 100 ms、通知间隔大于 2 s、回调丢包或解析失败
...
列和训练 25 Hz 派生合同。标注端同时接受 1.5 与 1.6，但所有新采集必须写 1.6。
