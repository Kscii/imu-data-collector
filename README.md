...
合成运动人工审核的已定模块边界和实施待办见[合成运动标注模块](docs/synthetic-motion-module.md)。该模块已有本地 pilot，尚未上线；真实设备录制与现有标注流程保持独立。
...
客户交付不改变训练 HDF5 合同：平台按不可变训练快照后台生成包含合并 H5、逐录制 MP4、冻结
...
本轮冻结的数据生命周期、三态任务、25 Hz 派生和服务器配置边界见
...
macOS 安装后从“应用程序”打开 `IMU Data Collector.app`，它作为菜单栏应用运行且不显示 Dock
图标；菜单可重新打开 WebUI、数据目录和日志目录。当前 DMG 使用 ad-hoc 签名且未公证，首次
...
的“记录与发布”只负责查看后台收尾/上传、人工重试和测试数据手动交接；标注、同步、aligned
...
- 正式 25 Hz 导出同时要求 `prod`、同步 verified、标注定稿、源哈希不变以及加速度计/陀螺仪
...
- 九名白名单成员采用同权领取：只有当前负责人可编辑，其他成员可显式接管；完成后负责人或
  管理员可重开。标签管理域内所有白名单成员同权，可以创建、改名、启停、迁移和删除未使用
  标签；每次变更生成带操作者、时间和操作类型的不可变 taxonomy 版本。录制删除必须二次输入
  `DELETE <recording_id>`，管理员清理历史训练快照必须
