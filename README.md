...
- 九名白名单成员可以删除尚未进入有效训练发布的录制，但必须二次输入
  `DELETE <recording_id>`；应用立即隐藏并删除活动对象，GCS 存储桶保留 7 天软删除恢复能力。
  有效发布可先用 `REVOKE <release_id>` 和原因撤销；撤销后立即停止展示与下载，发布 TAR、清单
  同样进入 GCS 软删除，轻量墓碑继续保留。
- `captures/` 下超过 7 天仍没有 manifest 的中断上传由每日任务清理；完整录制和有 manifest 的
  对象绝不由该任务自动删除。
